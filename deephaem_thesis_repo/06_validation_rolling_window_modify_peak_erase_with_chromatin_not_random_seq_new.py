...
#SIMONE'S CODE INSTEAD:

#all sequence

regionOI = 'chr1:1379046-1418981'
...
region_to_edit = 'chr1:1396946-1401081'
...
#GETTING FLAT PART FROM REGION 3 (4135 bp so let's say 242067680-242071815)

regionOI = 'chr2:242067680-242071815'
...
region_pybedtools = region_pybedtools.sequence(fi=genome_fasta)
list_fasta = open(region_pybedtools.seqfn).read().split('\n')[:-1]
sequence_edited_full = [list_fasta[i].upper() for i in range(1, df.shape[0]*2, 2)]
sequence_edited = sequence_edited_full[0]
...
for region in tqdm(regions_to_loop):
    ...
    sequences = [final_sequence[regions[0][1]:regions[0][2]]]
    #
...
for col in pred_df.columns:
    ...
    ax[2].set_title("")
    plt.show()
    fig.savefig('/well/PROCARDIS/domwest/deephaem_prep/input_files_for_deephaem/redo_modified_peak_rolling_window/validation_rolling_window_peak_erase_region1_prediction_all_bases_' + col + '.png', bbox_inches='tight')
    print('@'*100)
    print('@'*100)
...
for col in pred_df.columns:
    ...
    plt.show()
    fig.savefig('/well/PROCARDIS/domwest/deephaem_prep/input_files_for_deephaem/redo_modified_peak_rolling_window/validation_rolling_window_peak_erase_only_overlay_region1_prediction_all_bases_' + col + '.png', bbox_inches='tight')
    print('@'*100)