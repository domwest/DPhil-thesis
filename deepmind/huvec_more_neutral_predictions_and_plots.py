...
df_targets.head(3)
# df_targets.shape[0]
# df_targets['description'].iloc[4767]

# df_targets_filt = df_targets_filt[(df_targets_filt.index.isin([61, 211]))]
...
def plot_tracks(tracks, interval, classif, chrom, pos, ref, alt, var_type, height=1.5):
  ...
  plt.tight_layout()
  plt.savefig('/well/PROCARDIS/domwest/deepmind/plot_tracks_huvec_more_neutral/' + classif + '_' + chrom + '_' + str(pos) + '_' + ref + '_' + alt + '_' + var_type + '_tracks.png')
...
def score_the_variant(chrom, pos, ref, alt, rsid, classifier, index, var_type):
  ...
  #
  pred_df.to_csv('/well/PROCARDIS/domwest/deepmind/huvec_more_neutral_prediction_dfs/' + classif + '_' + chrom + '_' + str(pos) + '_' + ref + '_' + alt + '.csv')

score_the_variant(sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[1], 'DNASE:endothelial cell of umbilical vein newborn 1', 29, 'huvec_neutral')
score_the_variant(sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[1], 'DNASE:endothelial cell of umbilical vein newborn 2', 118, 'huvec_neutral')
